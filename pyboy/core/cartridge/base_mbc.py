...
class BaseMBC:
    ...
    def overrideitem(self, rom_bank, address, value):
        if 0x0000 <= address < 0x4000:
            logger.debug(
                "Performing overwrite on address: 0x{rom_bank:04x}:0x{address:04x}. New value: 0x{value:04x} Old value: 0x{self.rombanks[rom_bank][address]:04x}"
            )
