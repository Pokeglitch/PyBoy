...
class MBC3(BaseMBC):
    def setitem(self, address, value):
        # logger.error3("TEST: 0x\n", 1,2)
        # logger.error2("TEST: 0x\n")
        # logger.error2("TEST: 0x%0.4x\n", address)
        if 0x0000 <= address < 0x2000:
            ...
            if self.rambank_enabled:
                if self.rambank_selected <= 0x03:
                    ...
                    logger.error2("Invalid RAM bank selected: 0x%0.2x", self.rambank_selected)
        else:
            logger.error2("Invalid writing address: 0x%0.4x", address)
